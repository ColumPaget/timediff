...
void SettingsInit(int argc, const char *argv[])
{
    ...
    while (item)
    {
        if (strcmp(item, "-min")==0) Settings.ClipMin=atof(CommandLineNext(Cmd));
        else if (strcmp(item, "-max")==0) Settings.ClipMax=atof(CommandLineNext(Cmd));
        else if (strcmp(item, "-w")==0) Settings.WarnTime=atof(CommandLineNext(Cmd));
